...
3rd Inf Div(US)와 12th Pz Div(서독)는 예하 부대가 여럿이라 사단 단위 박스로 묶어 각 부대별 트랙을 별도로 추적 (칸 클릭형):

| 3rd Inf Div 예하 부대 | RP 범위 | CP 범위 |
|---|---|---|
| Fwd HQ | 추적 안 함 | 0–1 (CP만 추적) |
| Mn HQ | 0–1 | 0–1 |
| 1/3 | 0–1 | 0–1 |
| 2/3 | 0–1 | 0–1 |
| 3/3 | 0–1 | 0–1 |
...
- HQ별 RP/CP, Offmap CP/RP, 사단 예하 부대별 RP/CP: 트랙 칸을 가로로 나열한 클릭형 위젯. 현재 값 칸이 강조 표시되고, 다른 칸을 클릭하면 그 값으로 즉시 이동 (보드게임 마커 이동과 동일한 멘탈 모델)
- CSP, VP: `−` `값` `+` 형태의 인라인 카운터. 0 미만으로 내려가지 않음 (상한 없음, 단 음수 방지)
- 패널 내부는 의미 단위로 테두리 박스로 구분: NATO는 (단일 HQ 목록) / (3rd Inf Div 예하 부대) / (12th Pz Div 예하 부대) / (Offmap+SAM Strength) / (CSP) / (VP) 박스, WP는 (Offmap+SAM Strength) / (CSP) / (VP) 박스
