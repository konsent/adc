...
const MOVEMENT = {
  ...
  front: {
    ...
    9: ['H', 'HL', 'DD', 'DL', 'H', 'HL'], 10: ['H', 'HL', 'DD', 'DR', 'H', 'HL'],
  },
};
...
function applyToken(state, token) {
  ...
  for (const symbol of token.match(/[HCLRD]/g) ?? []) {
    ...
    if (symbol === 'C') next = { ...next, alt: next.alt + 1 };
...
export function moveSpohRandomOpponent(target, { attacker, die }) {
  ...
  let actions = MOVEMENT[column(attacker, target)][die].slice(0, Math.max(0, Math.floor(target.speed) - (damaged ? 1 : 0)));
  // T-3 원문: 손상 기체는 한 행동을 덜 하고 모든 C 코드를 H로 바꾼다.
  if (damaged) actions = actions.map(token => token.replaceAll('C', 'H'));
  let state = { ...target, hex: { ...target.hex }, position: { ...target.position } };
  for (const token of actions) state = applyToken(state, token);
